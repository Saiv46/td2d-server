...
class TailsProjectile extends BaseEntity {
	...
	static validate (map, params, session) {
		assert.deepEqual(Math.abs(direction), 1, 'TPROJ_DIRECTION')
		assert(damage && damage <= TailsCapabilities.projectileMaxDamage, 'TPROJ_DAMAGE')
		assert(charge && charge <= TailsCapabilities.projectileMaxCharge, 'TPROJ_CHARGE')
